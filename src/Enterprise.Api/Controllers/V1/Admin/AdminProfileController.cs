...
[RequireAdmin]
public sealed class AdminProfileController : ApiControllerBase
{
    [HttpGet]
    ...
    public async Task<ActionResult<ApiResponse<ProfileDto>>> Update(
        [FromBody] AdminUpdateProfileRequest request, CancellationToken cancellationToken) =>
        OkResponse(
    ...
    public async Task<ActionResult<ApiResponse<object?>>> RequestChangeEmail(
        [FromBody] AdminChangeEmailRequest request, CancellationToken cancellationToken)
    {
    ...
    public async Task<ActionResult<ApiResponse<ProfileDto>>> ConfirmChangeEmail(
        [FromBody] AdminConfirmChangeEmailRequest request, CancellationToken cancellationToken) =>
        OkResponse(
...
public sealed record AdminUpdateProfileRequest(string FirstName, string LastName);
public sealed record AdminChangeEmailRequest(string NewEmail);
public sealed record AdminConfirmChangeEmailRequest(string NewEmail, string Otp);
