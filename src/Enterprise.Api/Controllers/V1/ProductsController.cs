...
/// <summary>
/// Demonstrates every "API" requirement from the brief in one feature: pagination, filtering,
/// sorting and search are all just properties on <see cref="GetProductsListQuery"/> bound
/// straight from the query string - there is no separate "search DTO" or manual query-string
/// parsing here.
/// </summary>
[ApiVersion("1.0")]
...
public sealed class ProductsController : ApiControllerBase
{
    ...
    public async Task<IActionResult> GetById(
        Guid id, [FromQuery] bool? allLanguages, CancellationToken cancellationToken)
    {
        var wantsAllLanguages = allLanguages ?? (User.IsInRole("Admin") || User.HasClaim("permission", "Products.Update"));
        if (wantsAllLanguages)
        {
            var adminProduct = await Mediator.Send(new GetAdminProductByIdQuery(id), cancellationToken);
            return OkResponse(adminProduct, MessageKeys.Product.Retrieved).Result!;
        }

        var clientProduct = await Mediator.Send(new GetProductByIdQuery(id), cancellationToken);
        return OkResponse(clientProduct, MessageKeys.Product.Retrieved).Result!;
    }
...
/// <summary>Route-bound Id is never trusted from the request body, so it isn't part of this
/// contract - the command combines it with the route value in the controller action.</summary>
